...
#     if content and ord(content[0]) >= 0x0600 and ord(content[0]) <= 0x06FF:
...
def main():
    ...
    save_button = tk.Button(frame, text = "حفظ",command = lambda: save_file(window, text_edit))
    open_button = tk.Button(frame, text = "فتح",command = lambda: open_file(window, text_edit))
    say_button = tk.Button(frame, text = "اقرأ",command = lambda: read_text(text_edit))
